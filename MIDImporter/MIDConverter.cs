...
{
    private static readonly string executablePath = Path.Combine("rml_mods", "midi_importer");
    ...
    private static Task PerformWindowsUnpack(string band, string inputFile, string outputPath, string outputName, bool showDebugWindow)
    {
        ...
        if (!File.Exists(windowsExecutablePath))
            throw new FileNotFoundException("Could not find fluidsynth. Is it present under rml_mods/midi_converter/fluidsynth.exe?");
