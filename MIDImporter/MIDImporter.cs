...
public class MIDImporter : ResoniteMod
{
    ...
    private static readonly string bandPath = Path.Combine("rml_mods", "midi_importer", "bands");
    private static readonly string defaultBandName = "GeneralUser_GS_v1.471.sf2";
    private static ModConfiguration config;

    [AutoRegisterConfigKey]
    public readonly static ModConfigurationKey<bool> importAsRawFiles =
        new("importAsRawFiles",
        "Import binary variants of audio files",
        () => false);
    [AutoRegisterConfigKey]
    public readonly static ModConfigurationKey<bool> showDebugWindow =
        new("showDebugWindow",
        "Show debug window during import",
        () => false);
    [AutoRegisterConfigKey]
    public readonly static ModConfigurationKey<string> bandName = 
        new("band", 
    ...
    public class UniversalImporterPatch
    {
        static bool Prefix(IEnumerable<string> files, ref Task __result, World world)
        {
            ...
            else
            {
                ...
                foreach (var file in files)
                {
                    ...
                    preConvertedslot.GlobalPosition += offset;
                    UniversalImporter.ImportRawFile(preConvertedslot, file);
                    offset += new float3(0.2f, 0f, 0f);
                }
            }
            return true;
        }
        ...
        if (!File.Exists(Path.Combine(bandPath, config.GetValue(bandName))))
        {
            ...
            {
                Error($"The default MID converter {defaultBandName} was not found. Are you missing this file under rml_mods/midi_converter/bands?");
                return;
            }
